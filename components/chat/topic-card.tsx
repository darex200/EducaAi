...
export function TopicCard({
  ...
  return (
    <button
      ...
      type="button"
      onClick={onSelect}
      className={`btn-hover-lift w-full rounded-xl border px-3 py-2.5 text-left duration-300 ${
        isDarkMode ? "btn-hover-lift-dark" : "btn-hover-lift-light"
      } ${
        isActive
          ? `topic-card-active-pulse ${
              isDarkMode
                ? "border-blue-500/60 bg-blue-950/50 ring-1 ring-blue-500/40"
    ...
    >
      <p className={`text-sm font-semibold transition-colors ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}>
        {topic.title}
      </p>
